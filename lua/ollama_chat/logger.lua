...
function M.setup()
    ...
    if not ok then
        ...
        return
    elseif not config_logging then
        vim.notify("OllamaClient Logger: Logging not configured and/or not enabled: " .. tostring(err), vim.log_levels.ERROR)
        return
    end
...
function M.error(message)
    if state.log_level_num >= log_level.ERROR then
...
function M.except(message)
    if state.log_level_num >= log_level.EXCEPT then
